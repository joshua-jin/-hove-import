...
class ActiveAgentAppCountExecutor (taskIndex: Int, threadBound: Int, filePrefix: String)
                            extends BasicExecutor(taskIndex, threadBound, filePrefix) {
  override def getTable(): BufferedMutator = {
    return HBaseService.getConnection.getBufferedMutator(TableName.valueOf("imei-360-app"))
  }
  ...
  override def setupPuts(lineData: String, puts: util.List[Put], putIndex: Int, noDataCount: Int): (Int, Int) = {
    ...
    if (dataArray.length >= 2) {
      ...
      try {
        ...
        p.add(fCommonColumn, Bytes.toBytes("active-agent-app-count"), Bytes.toBytes(activeAppCount))
        puts.add(p)
        return (1, 0)
      } catch {
