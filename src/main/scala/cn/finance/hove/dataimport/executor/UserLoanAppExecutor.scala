...
class UserLoanAppExecutor(taskIndex: Int, threadBound: Int, filePrefix: String)
  ...
  override def setupPuts(lineData: String, puts: util.List[Put], putIndex: Int, noDataCount: Int): (Int, Int) = {
    ...
    if (dataArray.length >= 2) {
      ...
      val loanAppCount = dataArray(1)
      try {
        val p = new Put(Hex.decodeHex(imeiMD5Str.toCharArray))
        p.add(fCommonColumn, Bytes.toBytes("active-loan-app"), Bytes.toBytes(loanAppCount))
        puts.add(p)
        return (1, 0)
      } catch {
